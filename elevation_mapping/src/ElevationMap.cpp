...
//      fusedMap_({"elevation", "upper_bound", "lower_bound", "color"}),
      rawMap_({"elevation", "variance", "horizontal_variance_x", "horizontal_variance_y", "horizontal_variance_xy", "color", "time", "lowest_scan_point", "sensor_x_at_lowest_scan", "sensor_y_at_lowest_scan", "sensor_z_at_lowest_scan", "roll", "pitch"}),
      fusedMap_({"elevation", "upper_bound", "lower_bound", "color", "roll", "pitch"}),
...
bool ElevationMap::add(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances,
                       const ros::Time& timestamp, const Eigen::Affine3d& transformationSensorToMap, PlaneDetection &planeArray_) {
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i) {
    ...
    auto& roll = rawMap_.at("roll", index);
    auto& pitch = rawMap_.at("pitch", index);

//    planeArray_.getPitchRoll(i, roll, pitch);
    ...
    // Deal with multiple inclines in one cell

    // Get roll and pitch of given point
    float pointRoll, pointPitch;
    planeArray_.getPitchRoll(i, pointRoll, pointPitch);


    // Deal with multiple heights in one cell.
    double mahalanobisDistance = fabs(point.z - elevation) / sqrt(variance);
    double mahalanobisDistanceRoll = fabs(pointRoll - roll) / sqrt(variance);
    double mahalanobisDistancePitch = fabs(pointPitch - pitch) / sqrt(variance);


    if(std::isnan(mahalanobisDistance))
    {
        mahalanobisDistance = std::numeric_limits<double>::infinity();
        mahalanobisDistanceRoll = std::numeric_limits<double>::infinity();
        mahalanobisDistancePitch = std::numeric_limits<double>::infinity();
    ...
    if (mahalanobisDistanceRoll > mahalanobisDistanceThreshold_ || mahalanobisDistancePitch > mahalanobisDistanceThreshold_) {
      if (scanTimeSinceInitialization - time <= scanningDuration_ &&
          elevation > point.z) {  // NOLINT(cppcoreguidelines-pro-type-union-access)
        // Ignore point if measurement is from the same point cloud (time comparison) and
        // if measurement is lower then the elevation in the map.
      } else if (scanTimeSinceInitialization - time <= scanningDuration_ && (pointRoll < M_PI/2 || pointPitch < M_PI/2) ) {
        // If point is higher.
        roll = pointRoll;
        pitch = pointPitch;
      }
...
//    roll =
//        (variance * pointRoll + pointVariance * roll) / (variance + pointVariance);
//    pitch =
//            (variance * pointPitch+ pointVariance * pitch) / (variance + pointVariance);
...
void ElevationMap::setRawSubmapHeight(const grid_map::Position& initPosition, float mapHeight, float mapRoll, float mapPitch, double lengthInXSubmap,
  ...
  const grid_map::Position topLeftPosition(initPosition(0) + lengthInYSubmap / 2, initPosition(1) + lengthInXSubmap / 2);
  ...
  grid_map::Matrix& rollData = rawMap_["roll"];
  grid_map::Matrix& pitchData = rawMap_["pitch"];
  ...
  for (grid_map::SubmapIterator iterator(rawMap_, submapTopLeftIndex, submapBufferSize); !iterator.isPastEnd(); ++iterator) {
    ...
    rollData(index(0), index(1)) = mapRoll;
    pitchData(index(0), index(1)) = mapPitch;
