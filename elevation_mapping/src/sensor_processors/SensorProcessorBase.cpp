...
bool SensorProcessorBase::readParameters() {
  ...
  bool useTorsoLidar;
  nodeHandle_.param("use_torso_lidar", useTorsoLidar, false);

  if(useTorsoLidar)
  {
      nodeHandle_.param("sensor_frame_id_torso", sensorFrameId_, std::string("/sensor"));  // TODO(max): Fail if parameters are not found.
