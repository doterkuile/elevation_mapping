...
ElevationMapping::ElevationMapping(ros::NodeHandle& nodeHandle)
  ...
  readParameters();
  pointCloudTorsoSubscriber_ = nodeHandle_.subscribe(pointCloudTorsoTopic_, 1, &ElevationMapping::pointCloudTorsoCallback, this);
...
bool ElevationMapping::readParameters() {
    ...
    bool useTorsoLidar;
    nodeHandle_.param("use_torso_lidar", useTorsoLidar, true);


  // ElevationMapping parameters.
    if(useTorsoLidar)
    {
        nodeHandle_.param("point_cloud_topic_torso", pointCloudTopic_, std::string("/points"));
...
bool ElevationMapping::initialize() {
  ...
  torsoCloud_ = boost::make_shared<pcl::PointCloud<pcl::PointXYZRGB>> ();
...
void ElevationMapping::pointCloudTorsoCallback(const sensor_msgs::PointCloud2  &rawPointCloud )
{

    boost::lock_guard<boost::recursive_mutex> guard(cloudTorsoMutex_);
    pcl::PCLPointCloud2 pcl_pc;
    pcl_conversions::toPCL(rawPointCloud, pcl_pc);
    pcl::fromPCLPointCloud2(pcl_pc, *torsoCloud_);
...
//  sensor_msgs::PointCloud2 rawPointCloud;
//  pcl::toROSMsg(*inputCloud,rawPointCloud);
  ...
  planeArray_.setCloud(pointCloudProcessed);
  ...
  ROS_INFO("Inside pointcloud callback");
  // Add point cloud to elevation map.
  if (!map_.add(pointCloudProcessed, measurementVariances, lastPointCloudUpdateTime_,
                Eigen::Affine3d(sensorProcessor_->transformationSensorToMap_), planeArray_)) {
...
bool ElevationMapping::initializeElevationMap() {
  if (initializeElevationMap_) {
    if (static_cast<elevation_mapping::InitializationMethods>(initializationMethod_) ==
      ...
      try {
        transformListener_.waitForTransform(mapFrameId_, targetFrameInitSubmap_, ros::Time(0), ros::Duration(15.0));
        ...
        eQuaternion q(transform.getRotation());
        eVector3 rpy;
        extractRollPitchYaw(q, &rpy.x(), &rpy.y(), &rpy.z());
        ...
        map_.setRawSubmapHeight(positionRobot, transform.getOrigin().z() + initSubmapHeightOffset_, rpy.x(), rpy.y(), lengthInXInitSubmap_,
