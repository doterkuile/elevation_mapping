...
class ElevationMapping {
 public:
   ...
   */
  void pointCloudCallback(const  sensor_msgs::PointCloud2& pointCloud);//pcl::PointCloud<pcl::PointXYZRGB>::Ptr &inputCloud);

  void pointCloudTorsoCallback(const  sensor_msgs::PointCloud2& pointCloud);//pcl::PointCloud<pcl::PointXYZRGB>::Ptr &inputCloud);
 ...
 private:
  ...
  ros::Subscriber pointCloudTorsoSubscriber_;
  ...
  std::string pointCloudTorsoTopic_;
  ...
  //! Obstacle vector
  PlaneDetection planeArray_;

  //! Pointcloud of Torso
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr torsoCloud_;

  //! Mutex for pointcloud of torso
  boost::recursive_mutex cloudTorsoMutex_;
